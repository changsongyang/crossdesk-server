...
公网 IP 的地理位置默认不访问外部服务，内网、回环和 Docker 私有网段会显示为 `Private network`。如需开启公网 GeoIP 查询，可设置 `CROSSDESK_GEOIP_LOOKUP=1` 并通过 `CROSSDESK_GEOIP_TOKEN` 配置 IPinfo Lite token；默认请求 `https://api.ipinfo.io/lite/{ip}?token={token}`。IPinfo Lite 返回国家信息，不返回省市，因此中国省份地图会将这类用户计入未解析。如需切换到自建或其他 IPinfo Lite 格式接口，可通过 `CROSSDESK_GEOIP_SCHEME`、`CROSSDESK_GEOIP_HOST`、`CROSSDESK_GEOIP_PORT` 和 `CROSSDESK_GEOIP_PATH` 配置查询端点，其中路径里的 `{ip}` 和 `{token}` 会被替换。解析仅使用 IPinfo Lite 的 `country`、`country_code` 字段。查询超时时间可通过 `CROSSDESK_GEOIP_TIMEOUT_MS` 调整，默认 1200ms。
