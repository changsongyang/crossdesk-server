...
Public IP geolocation is queried from `ip-api.com` by default and cached in the server process. Private, loopback, and Docker private-network addresses are shown as `Private network`. Set `CROSSDESK_GEOIP_LOOKUP=0` to disable public GeoIP lookups. The lookup timeout can be adjusted with `CROSSDESK_GEOIP_TIMEOUT_MS`; the default is 1200ms.
